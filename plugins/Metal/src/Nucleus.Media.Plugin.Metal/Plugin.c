...
#include "Nucleus.Media.Plugin.Metal/Plugin.h"
    ...
    )
{
    ...
    //
    Nucleus_Media_Plugin_Metal_VideoSystemFactory *factory;
    status = Nucleus_Media_Plugin_Metal_VideoSystemFactory_create(&factory);
    if (Nucleus_Unlikely(status)) return status;
    //
...
constructDispatch
    (
        Nucleus_Media_Plugin_Metal_Plugin_Class *dispatch
    )
{
    ...
    )
{
    ...
    Nucleus_Type *type;
    Nucleus_Status status;
    status = Nucleus_Media_Plugin_Metal_Plugin_getType(&type);
    if (Nucleus_Unlikely(status)) return status;
    status = Nucleus_Media_Plugin_construct(NUCLEUS_MEDIA_PLUGIN(self));
    if (Nucleus_Unlikely(status)) return status;
    //
    status = Nucleus_String_create(&self->name, u8"Nucleus: Media Metal Plugin");
    if (Nucleus_Unlikely(status)) return status;
    //
...
Nucleus_DefineDefaultCreate(Nucleus_Media_Plugin_Metal_Plugin)
