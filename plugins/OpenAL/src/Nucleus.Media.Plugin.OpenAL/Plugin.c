    ...
    )
{
    ...
    //
    Nucleus_Media_Plugin_OpenAL_AudioSystemFactory *factory;
    status = Nucleus_Media_Plugin_OpenAL_AudioSystemFactory_create(&factory);
    if (Nucleus_Unlikely(status)) return status;
    //
    status = Nucleus_MediaContext_registerAudioSystemFactory(context, NUCLEUS_MEDIA_AUDIOSYSTEMFACTORY(factory));
    ...
    )
{
    ...
    Nucleus_Type *type;
    Nucleus_Status status;
    status = Nucleus_Media_Plugin_OpenAL_Plugin_getType(&type);
    if (Nucleus_Unlikely(status)) return status;
    status = Nucleus_Media_Plugin_construct(NUCLEUS_MEDIA_PLUGIN(self));
    if (Nucleus_Unlikely(status)) return status;
    //
    status = Nucleus_String_create(&self->name, u8"Nucleus: Media OpenAL Plugin");
    if (Nucleus_Unlikely(status)) return status;
    //
    ...
    )
{
    ...
    status = Nucleus_Object_allocate((Nucleus_Object **)&temporary, 
                                     sizeof(Nucleus_Media_Plugin_OpenAL_Plugin));
    if (Nucleus_Unlikely(status)) return status;
    status = Nucleus_Media_Plugin_OpenAL_Plugin_construct(temporary);
    if (Nucleus_Unlikely(status))
    {
